...
import { ExtractionResultSchema, InMemoryAuditLogger, extractRfInternalPassport, validateDeptCode, validatePassportNumber } from "../index.js";
const ORIGINAL_ENDPOINT = process.env.ONLINE_OCR_ENDPOINT;
afterEach(() => {
    process.env.ONLINE_OCR_ENDPOINT = ORIGINAL_ENDPOINT;
});
function buildInput(layout) {
...
describe("KEIScore foundation", () => {
    ...
    it("extracts all fields on ideal scan", async () => {
        ...
        const result = await extractRfInternalPassport(buildInput({
            width: 2800,
            height: 2000,
    ...
    it("uses pass cascade for 15 degree skew", async () => {
        ...
        expect(result.passport_number).toBe("4120 №093363");
        expect(result.field_reports.find((report) => report.field === "passport_number")?.pass_id).toBe("B");
    });
...
//# sourceMappingURL=passport.spec.js.map