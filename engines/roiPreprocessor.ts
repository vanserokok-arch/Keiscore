...
export async function preprocessRoiForOcr(inputPath: string): Promise<string> {
  return preprocessRoiForOcrWithConfig(inputPath, {});
}

export async function preprocessRoiForOcrWithConfig(
  inputPath: string,
  ...
  const paddedBounds = withPadding(
...
function withPadding(bounds: Bounds, maxWidth: number, maxHeight: number, paddingRatio: number): Bounds {
...
async function autoCropNonWhite(buffer: Buffer): Promise<Buffer> {
  let width = 1;
  let height = 1;
  let raw: Buffer;
  try {
...
async function centerAndIncreaseHeight(buffer: Buffer): Promise<Buffer> {
  try {
...
function isNarrowTextField(field: PassportField | undefined): boolean {
  return field === "passport_number" || field === "dept_code";
}

function computeTargetWidth(field: PassportField | undefined, sourceWidth: number): number {
