...
export class AnchorModel {
  static async findAnchors(
    ...
    const sameLine = (a: MockOcrToken, b: MockOcrToken): boolean => {
      const ay = a.bbox.y1;
      const by = b.bbox.y1;
    ...
    const sortedByLine = [...normalizedTokens].sort((a, b) => {
      if (a.token.bbox.y1 !== b.token.bbox.y1) return a.token.bbox.y1 - b.token.bbox.y1;
      return a.token.bbox.x1 - b.token.bbox.x1;
    });

    for (const anchorWord of ANCHOR_WORDS) {
      if (!anchorWord.includes(" ")) continue;
      if (anchors[anchorWord] !== undefined) continue;
      ...
      for (let i = 0; i < sortedByLine.length; i++) {
        ...
        if (!(start.text === first || start.text.startsWith(first))) continue;
...
function computeBaselineY(tokens: MockOcrToken[], anchors: AnchorResult["anchors"]): number | null {
...
function computeLineHeight(tokens: MockOcrToken[]): number {
  if (tokens.length === 0) {
    return 40;
  }
...
function computeTextLines(tokens: MockOcrToken[]): number[] {
  const ys = tokens.map((token) => token.bbox.y1).sort((a, b) => a - b);
  if (ys.length === 0) {
    return [];
  }
  const lines: number[] = [ys[0] ?? 0];
...
async function getOcrTokens(input: NormalizedInput, logger: AuditLogger): Promise<MockOcrToken[]> {
  const mockTokens = input.mockLayout?.ocrTokens;
  if (mockTokens !== undefined) return mockTokens;
  ...
  try {
    const { stdout } = await execa(
      "tesseract",
      [imagePath, "stdout", "-l", "rus", "--psm", "6", "tsv"],
...
async function getCenterText(input: NormalizedInput, logger: AuditLogger): Promise<string> {
  // тестовый путь (оставляем как был)
  const mock =
    input.mockLayout?.centralWindowText ??
...
function classifyByText(text: string): AnchorResult["pageType"] {
  ...
  const hasSpreadLayoutHint =
    normalizedText.includes("КОД ПОДРАЗДЕЛЕНИЯ") ||
  ...
  if (
    normalizedText.includes("МЕСТО ЖИТЕЛЬСТВА") ||
...
function buildFallbackGridAnchors(
  input: NormalizedInput,
...
function getAnchorPrefixes(anchorWord: string): string[] {
