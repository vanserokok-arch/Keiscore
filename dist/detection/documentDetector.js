...
function clampBBox(box, imgW, imgH) {
 ...
 */
async function computeNonWhiteBBox(imagePath) {
    const targetW = 1400;
 ...
 */
async function detectByContourHeuristics(imagePath) {
...
async function analyzeOcrHints(imagePath) {
    try {
        const meta = await sharp(imagePath).metadata();
...
function buildDocumentDetectionFailed(aspectRatio, areaRatio, contour) {
    const out = {
        detected: false,
        docType: "UNKNOWN",
        confidence: 0
    ...
    if (areaRatio !== undefined)
        out.areaRatio = areaRatio;
    if (contour !== undefined)
        out.contour = contour;
    return out;
}
function buildDocumentDetectionSuccess(contour, aspectRatio, areaRatio, confidence = 0.75) {
    const out = {
        detected: true,
...
export class DocumentDetector {
    static async detect(input, logger) {
        ...
        const areaRatio = bbox && imgW && imgH ? (bbox.width * bbox.height) / (imgW * imgH) : undefined;
        ...
        logger.log({
            ...
            data: {
                ...
                matchedStrict: false,
                matchedSpread: fallbackOcr.hasRussia &&
        ...
        const ocrSpreadDetected = fallbackOcr.hasRussia &&
            fallbackOcr.hasFederation &&
            fallbackOcr.hasMrz &&
            fallbackOcr.hasSpreadHints;
