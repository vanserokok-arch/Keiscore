...
export class FormatNormalizer {
    static async normalize(input, opts, logger) {
        ...
        if (input.kind === "path") {
            ...
            const normalizedGeometry = await inferImageGeometry(normalizedBuffer, imageGeometry.width, imageGeometry.height);
            ...
            if (mime === "application/pdf") {
                ...
                const syncedFirstPage = firstPage === undefined
    ...
    static async normalizePdfPath(sourcePath, opts, logger) {
        ...
        const requestedRange = opts?.pdfPageRange === undefined
            ? null
        ...
        try {
            ...
            const pages = await Promise.all(renderedPngNames.map(async (fileName, index) => {
                ...
                return {
                    pageNumber: Number.isFinite(filePageNumber) && filePageNumber > 0 ? filePageNumber - 1 : fallbackPage,
                    imagePath,
    ...
    static async getPdfPageCount(sourcePath) {
    ...
    static async tryGetPdfPageCountWithPdfinfo(sourcePath) {
        try {
    ...
    static async tryGetPdfPageCountWithPdfJs(sourcePath) {
        ...
        try {
            ...
            const loadingTask = getDocumentCandidate({
...
function isStructuredCoreError(error) {
    return typeof error === "object" && error !== null && "coreError" in error;
}
function resolvePdfPageRange0Based(sourcePath, pageCount, requestedRange) {
    ...
    const isValid = Number.isInteger(from) &&
        Number.isInteger(to) &&
        from >= 0 &&
        to >= from &&
        from < pageCount &&
...
async function preprocessRasterPage(source, logger) {
    ...
    const contentCropped = crop.bbox === null
        ? adaptive.processed
    ...
    const passportTrimmed = passportCrop.bbox === null
        ? null
    ...
    const cropped = passportBbox === null
        ? contentCropped
    ...
    const preprocessing = {
        applied: orientation.rotationDeg !== 0 ||
...
async function computePassportForegroundCrop(source, thresholdHint) {
...
async function trimPassportTailByDensity(source, bbox, threshold) {
    ...
    const trimRightCols = detectTrailingLowDensityRun({
        ...
        densityEps,
        measureDensity: (x) => countDarkInColumn(raw.data, imageW, imageH, x, top, bottom, threshold) / Math.max(1, localHeight)
    });
...
export function adaptiveThresholdRetryDecision(blackPixelRatio) {
...
function isTargetBlackPixelRatio(blackPixelRatio) {
    return blackPixelRatio >= 0.03 && blackPixelRatio <= 0.25;
}
function distanceToTargetRatio(blackPixelRatio) {
    if (isTargetBlackPixelRatio(blackPixelRatio))
        return 0;
    if (blackPixelRatio < 0.03)
        return 0.03 - blackPixelRatio;
    return blackPixelRatio - 0.25;
}
function extractRetryCount(strategy) {
...
async function lowerThresholdUntilTarget(source, seed, maxIterations, label = "lower_threshold") {
    let best = seed;
    let threshold = seed.threshold;
    let iteration = 0;
    while (iteration < maxIterations && threshold > 120 && !isTargetBlackPixelRatio(best.blackPixelRatio)) {
        if (best.blackPixelRatio <= 0.25)
            break;
        threshold = clampInt(threshold - 15, 120, 245);
        const pass = await evaluateAdaptiveCrop(source, `${label}_iter_${iteration + 1}_t${threshold}`, threshold);
        best = pass;
        iteration += 1;
    }
    return best;
}
async function evaluateAdaptiveCrop(source, thresholdStrategy, thresholdOverride) {
...
async function applyClaheLikeStretch(source, boost) {
...
async function applyMildUnsharp(source) {
...
function percentileByte(data, p) {
    const hist = new Array(256).fill(0);
    for (let i = 0; i < data.length; i += 1) {
        const v = data[i] ?? 0;
        hist[v] = (hist[v] ?? 0) + 1;
...
function findLargestComponentBbox(raw, width, height, threshold) {
...
function trimBboxByDensity(raw, width, height, threshold, bbox) {
...
function computeProjectionBbox(raw, width, height, threshold) {
...
function chooseTighterPassportBox(largest, trimmed, projected) {
    const candidates = [largest, trimmed, projected].filter((item) => item !== null);
    return candidates.sort((a, b) => a.area - b.area)[0] ?? largest;
}
function countDarkInColumn(raw, width, height, x, top, bottom, threshold) {
    if (x < 0 || x >= width) {
        return 0;
    }
    let count = 0;
...
function countDarkInRow(raw, width, left, right, y, threshold) {
    if (y < 0) {
        return 0;
    }
    let count = 0;
