...
export class ScoringEngine {
  static score(
    ...
    qualityMetrics?: ExtractionResult["quality_metrics"]
  ): ScoringSummary {
    ...
    for (const report of fieldReports) {
      const numericBoost =
        (report.field === "passport_number" || report.field === "dept_code") && report.validator_passed ? 0.18 : 0;
      const validatorSignal = report.validator_passed ? 1 : 0;
      if (report.validator_passed) {
        validatedCount += 1;
      }
      const score = clamp01(
        0.36 * clamp01(report.confidence) +
