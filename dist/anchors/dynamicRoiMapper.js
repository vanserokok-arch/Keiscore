...
export class DynamicROIMapper {
    static async map(input, detection, calibration, anchors, logger) {
        ...
        const makePercent = (x1, y1, x2, y2) => clampRoi({
        ...
        const issuedByRoi = isSpreadPage
            ? issuedAnchor === undefined
                ...
                : clampRoi({
                    ...
                    height: Math.round(Math.max(lineHeight * 4.2, Math.min(lineHeight * 5.6, height * 0.76 - Math.max(height * 0.47, issuedAnchor.y + lineHeight * 0.8)))),
                    page
                }, width, height)
            : issuedAnchor === undefined
