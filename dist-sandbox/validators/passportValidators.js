...
export function parseMrzLatinFio(value) {
    const lines = value
        .toUpperCase()
        .replace(/\r/gu, "\n")
        .split("\n")
...
export function transliterateMrzLatinToCyrillic(input) {
...
export function assessFioSurnameQuality(surname) {
...
function isSuspiciousFioWord(value) {
    if (value.length < 3) {
        return true;
    }
    if (/^(.)\1{2,}$/u.test(value)) {
...
function hasHighFioNoiseRatio(value) {
...
function passesFioSurnameQualityGuard(surname) {
    if (surname.length < 5) {
        return false;
    }
...
function normalizeMrzLine(input) {
    return input
        .replace(/[А]/gu, "A")
        .replace(/[В]/gu, "B")
        .replace(/[ЕЁ]/gu, "E")
...
function parseSingleMrzLine(line) {
