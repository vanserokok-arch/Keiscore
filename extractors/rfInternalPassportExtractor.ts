...
export class RfInternalPassportExtractor {
  static async extract(input: InputFile, opts?: ExtractOptions): Promise<ExtractionResult> {
    ...
    try {
      ...
      const anchorsForMapping: AnchorResult = anchors;
...
function validateByPassOrder(
  field: PassportField,
  attempts: OcrPassResult[]
): (OcrPassResult & { text: string }) | null {
...
function validateByPassOrderDetailed(
  field: PassportField,
  attempts: OcrPassResult[]
): { validated: (OcrPassResult & { text: string }) | null; reason: string } {
...
function bestCandidatePreview(attempts: OcrPassResult[]): string {
  if (attempts.length === 0) {
    return "";
  }
  const best = [...attempts].sort((left, right) => right.confidence - left.confidence)[0];
...
async function buildPageFallbackAttempts(
  ...
  try {
    ...
    const issuedLine = buildTextLinesFromRows(rows).find(
      (line) =>
        line.text.length >= 18 &&
    ...
    if (issuedLine !== undefined) {
      ...
      const withMarker =
        /(ГУ|МВД|УФМС|ОТДЕЛ)/u.test(base) || base.length < 12 ? base : `ОТДЕЛ ${base}`;
      attempts.push({
        field: "issued_by",
        pass_id: "C",
        text: withMarker,
...
function parsePageTsvRows(tsv: string): Array<{ text: string; confidence: number; bbox: OcrPassResult["bbox"] }> {
  const rows = tsv.split(/\r?\n/u);
  const out: Array<{ text: string; confidence: number; bbox: OcrPassResult["bbox"] }> = [];
...
function buildTextLinesFromRows(
  rows: Array<{ text: string; confidence: number; bbox: OcrPassResult["bbox"] }>
