...
type RoiAuditContext = {
  pageType: AnchorResult["pageType"];
  usedFallbackGrid: boolean;
  anchorKeys: string[];
  textLineYs?: number[];
};

type FieldRoiWithAuditContext = FieldRoi & {
...
export class DynamicROIMapper {
  static async map(
    ...
    const roiMapperContext: RoiAuditContext = {
  ...
  static async attachRoiImagePaths(
    input: NormalizedInput,
    rois: FieldRoi[],
    logger: AuditLogger
  ): Promise<FieldRoi[]> {
    ...
    try {
      ...
      const mapped = await Promise.all(
        rois.map(async (fieldRoi) => {
          const fieldRoiWithContext = fieldRoi as FieldRoiWithAuditContext;
          const roiFilePath = join(
            tmpBase,
            `${fieldRoi.field}-${fieldRoi.roi.x}-${fieldRoi.roi.y}-${fieldRoi.roi.width}-${fieldRoi.roi.height}.png`
          );
          await sharp(normalizedBuffer)
            ...
            .png()
            .toFile(roiFilePath);
          const mappedFieldRoi: FieldRoiWithAuditContext = {
            ...fieldRoiWithContext,
          ...
          if (debugRoiDir) {
            const debugPath = join(
              debugRoiDir,
              `${Date.now()}_${mappedFieldRoi.roi.page}_${mappedFieldRoi.field}.png`
            );
            try {
      ...
      if (debugRoiDir) {
        const pageRoisMap = new Map<number, FieldRoiWithAuditContext[]>();
        for (const mappedRoi of mapped) {
          const pageRois = pageRoisMap.get(mappedRoi.roi.page) ?? [];
          pageRois.push(mappedRoi as FieldRoiWithAuditContext);
        ...
        await Promise.all(
          Array.from(pageRoisMap.entries()).map(async ([page, pageRois]) => {
            const ts = Date.now();
  ...
  static async cleanupRoiImagePaths(rois: FieldRoi[]): Promise<void> {
...
function snapToTextLine(y: number, textLineYs: number[] | undefined, lineHeight: number): number {
